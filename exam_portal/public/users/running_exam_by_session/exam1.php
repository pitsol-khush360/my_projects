...
<?php
	...
	if(!isset($_SESSION['seids']) && !isset($_SESSION['seid']))      // if first time loads.
	{
		...
		// echo '<script>alert("seids='.print_r($seids).'")</script>';
...
<html>
<head>
	<title>AbhyasClasses Live Exam</title>
	<meta charset="utf-8">
    	...
    	function timer() 
    	{
    		...
    		function run()
			{
					...
					// removing minutes from countdown that has considered already.
  					...
  					{
     					clearInterval(run);
     					window.location="section_controller1.php";  // redirect to controller
  					} 
  					else
  					{
...
<body onload="timer();next();">
	<div class="container-fluid">
		...
		<div class="row border border-top-0 border-left-0 border-right-0" style="font-size:1.1rem;">
			<div class="col-lg-6 col-sm-4 col-md-4 col-sm-12 border border-left-0 border-top-0 border-bottom-0">
							<div>
...
?>
								<?php
									if(isset($row['question_img']) && $row['question_img']!="")
									{
										...
										<img src="../../resources/{$img_path}" style="width:100%;height:150px;">
										</div>
question;
									}
								?>
...
<div class="container">
		...
		<!-- <div class="row">
			<button class="mx-auto mt-5 d-block" id="fullscreenbutton" onclick="viewPage()">Full Screen Mode</button>
		</div> -->
</div>
	...
	<script type="text/javascript">
		$("#nextbutton").on("click",			
			function()
			{
				answer=$('input[type="radio"]:checked').attr("value");

				if($('input[type="radio"]:checked'))
				{
				  	// deselecting previous radio button so that new radio button can be selected.
				  	$('input[type="radio"]:checked').prop("checked",false);
				}

				if(answer!="op1" && answer!="op2" && answer!="op3" && answer!="op4" && answer!="op5")
				{
					answer="none";
				}
			  	...
			  	{	
					...
					grid_que=$("[value="+(currentid-1)+"]").attr("id");  
				  	...
				  	// storing result values in session using php through ajax.

				  	$.get("next1.php?question="+question+"&answer="+answer,
				  		function(data,status1)
				  		{
				  			
				  			// includes() is used to check a substring is in string or not.returns true/false.
				  			// eg: data.includes("continue");
				  			//alert(data);
				  			//status=data.trim();
