...
<body>
<form action="displayCollectionList.php" method="post">
  ...
  <div class="input-group w-50" style = "float:right">
        ...
        <input type="text" class="form-control" placeholder="Search" name="searchfield" style="border: medium solid black;">
          ...
          <select name="key" id="keyword" placeholder="choose one"  class="btn btn-primary" style = "margin-left: 10px">
            <option value="catalogue_id">Collection Id</option>
            <option value="catalogue_seller_id">Seller Id</option>
            <option value="catalogue_Name">Collection Name</option>
            <option value="catalogue_status">Status</option>
          </select>
      </div>
...
$url=DOMAIN.'/rest/admin/getCollectionScreenRest.php';
    ...
    for($i=0;$i<$output['getcollectiondetails']['rows'];$i++)
    {
     $record.=<<< record
      <tr>
        ...
        <td>{$output['getcollectiondetails'][$i]['modification_datetime']}</td>
        <td>{$output['getcollectiondetails'][$i]['updatedby']}</td>
      </tr>      
record;
  }
echo $record;
    ...
    if(isset($output['getcollectiondetails']) && $output['getcollectiondetails']['response_code']==405){
      $record.=<<< record
      <h3>{$output['getcollectiondetails']['response_desc']}</h3>      
record;
      }
