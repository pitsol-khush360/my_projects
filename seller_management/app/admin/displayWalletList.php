...
<body>
  ...
  <form action="displayWalletList.php" method="post" style = "margin-top : 30px">
    ...
    for($i=0;$i<$output['getwalletdetails']['rows'];$i++)
    {
     $record.=<<< record
      <tr>
        ...
        <td style="text-align: right;">{$output['getwalletdetails'][$i]['opening_balance']}</td>
        <td style="text-align: right;">{$output['getwalletdetails'][$i]['closing_balance']}</td>
        <td>{$output['getwalletdetails'][$i]['balance_currency']}</td>
        <td>{$output['getwalletdetails'][$i]['value_date']}</td>
