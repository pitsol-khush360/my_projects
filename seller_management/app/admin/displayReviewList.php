...
<body>
<form action="displayReviewList.php" style = "margin-top : 30px">
<select id="pages" class="btn btn-primary" placeholder="">
  ...
  <div class="input-group w-50" style = "float:right">
        ...
        <select name="key" class="btn btn-primary" id="keyword" placeholder="choose one"  style = "margin-left: 10px">
...
$url=DOMAIN.'/rest/admin/getReviewListScreenRest.php';
 ...
 $record =''; 
  if(isset($output['getreviewdetails']) && $output['getreviewdetails']['response_code']==200)
  {
    for($i=0;$i<$output['getreviewdetails']['rows'];$i++)
    {
     $record.=<<< record
      <tr>
        <td>{$output['getreviewdetails'][$i]['id']}</td>
        <td>{$output['getreviewdetails'][$i]['seller_id']}</td>
        <td>{$output['getreviewdetails'][$i]['review_title']}</td>
        <td>{$output['getreviewdetails'][$i]['review']}</td>
        <td>{$output['getreviewdetails'][$i]['rating']}</td>
        <td>{$output['getreviewdetails'][$i]['creation_date_time']}</td>
...
echo $record;
    ...
    if(isset($output['getreviewdetails']) && $output['getreviewdetails']['response_code']==405){
      $record.=<<< record
        <h3>{$output['getreviewdetails']['response_desc']}</h3>
